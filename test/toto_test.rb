...
context Toto do
  ...
  context "GET to the archive" do
    ...
    context "through a year & month & day" do
      setup { @toto.get('/2009/12/11') }
      asserts("returns a 200")                      { topic.status }.equals 200
      should("includes the entries for that month") { topic.body }.includes_elements("li.entry", 1)
      should("includes the year & month & day")     { topic.body }.includes_html("h1" => /2009\/12\/11/)
    end
